...
ex:
    ...
    if (strcmp(parsed[0], "mov") == 0) // mov
    {
        ...
        if (op1ind)
        {
            ...
            { // we need a postfix
                if (postfix == 0)
                    printf("Error(0): suffix operator needed\n");
    ...
    else if (strcmp(parsed[0], "cmp") == 0) // cmp
    {
        uint32_t res = 0;
        if(op1ind && op2ind)
            printf("Error: too many memory references\n");
        ...
        if (op1ind)
        {
            ...
            {
                if (postfix == 0)
                    printf("Error(3): suffix operator neededn\n");
                // use postfix to specify size
                op1.value = memory_read_chunk(ins, op1.value, postfix);
            }
        }
        // cmp %eax, %ebx
        if(op1.bits != op2.bits && (op1.isRegister && op2.isRegister)) printf("Warning: comparing uneven sizes\n");
    ...
    else if (strcmp(parsed[0], "inc") == 0 || strcmp(parsed[0], "dec") == 0) // inc
    {
        ...
        if(op1ind)
        {
            if(postfix == 0) printf("Error: postfix needed\n");
